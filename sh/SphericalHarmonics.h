...
#define N_SH_CODES      647
  ...
  public:
    // init
    SphericalHarmonics();
    SphericalHarmonics(int code, int resolution, int colormap)  : resolution(resolution), colourmap(colormap) {
        loadCodes();
        ReadCode(code);
    }
    void setCode(int*code) {
        for (int i=0; i<8; i++) m[i]=(double)code[i];
        createList();
    }
    ...
    string getCode(int i) {
        return SphericHarmCodes[i % N_SH_CODES];
    }
    ...
    string fileName(string ext) {
        return string_format("sh-%s-%d-%d.%s", Code.c_str(), resolution, colourmap, ext.c_str());
    }
    ...
    void saveCTM() {
        ...
        vector <int32_t>idx;
        for (auto v : vertexes) append(vtx, xyz2fx3(v));
        for (auto n : normals) append(nrm,xyz2fx3(n));
        for (auto c : colors) append(cols, colour2fx4(c));
        for (auto ix : trigs) append(idx, ix);

        ctm.DefineMesh(vtx.data(), vtx.size()/3, (CTMuint*)idx.data(), idx.size()/3, nrm.data());
    ...
    void genTrigs() {
        ...
        for (int i=0; i < n - 1; i++) {
            for (int j=0; j < n - 1; j++)
                append(trigs, triangularize({(i + 1) * n + j, (i + 1) * n + j + 1,  i * n + j + 1, i * n + j}));
            append(trigs, triangularize({(i + 1) * n, (i + 1) * n + n - 1, i * n, i * n + n - 1}));
        }

        for (int i=0; i<n - 1; i++)
            append(trigs, triangularize({i, i + 1, n * (n - 1) + i + 1, n * (n - 1) + i}));
...
end_header
  ...
  private:
    ...
    XYZ Eval(double theta, double phi, double *m);
