...
void SphericalHarmonics::ReadCode(int code) { // read a code from the set. default SEQ.
...
XYZ SphericalHarmonics::Eval(double theta, double phi, double *m) {
    double r = 0;
    XYZ p;

    r += powc(sin(m[0] * phi), (double)m[1]);
    r += powc(cos(m[2] * phi), (double)m[3]);
    r += powc(sin(m[4] * theta), (double)m[5]);
    r += powc(cos(m[6] * theta), (double)m[7]);
...
void SphericalHarmonics::Paint(void) {
    ...
    for (int i = 0; i < resolution; i++) {
        ...
        for (int j = 0; j < resolution; j++) {
            v = j * dv;
            q[0] = Eval(u, v, m);
            n[0] = CalcNormal(q[0], Eval(u + du / 10, v, m), Eval(u, v + dv / 10, m));
            ...
            q[1] = Eval(u + du, v, m);
            n[1] = CalcNormal(q[1], Eval(u + du + du / 10, v, m),
                              Eval(u + du, v + dv / 10, m));
            ...
            q[2] = Eval(u + du, v + dv, m);
            n[2] = CalcNormal(q[2], Eval(u + du + du / 10, v + dv, m),
                              Eval(u + du, v + dv + dv / 10, m));
            ...
            q[3] = Eval(u, v + dv, m);
            n[3] = CalcNormal(q[3], Eval(u + du / 10, v + dv, m),
                              Eval(u, v + dv + dv / 10, m));
