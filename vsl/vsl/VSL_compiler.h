...
{
public:
  static double RadionicRate(std::wstring pw, int d)
  { // unicode strings
    if (pw.empty())
      return 0;
...
public:
  ...
  //      r = RadionicRate(string::fromStdWString(s), 8);
  //      r = FreqInOctave(r, 0);
...
public:
  class Symbols
  {
    ...
    symbolmap chars = {{L"+", PLUS}, {L"*", MULT}, {L"·", MULT}, {L"/", DIV}, {L"(", OPAREN}, {L")", CPAREN}, {L"{", OCURL}, {L"}", CCURL},
    {L"[", OSQARE}, {L"]", CSQUARE}, {L"\\", BACKSLASH}, {L"?", RANDOM}, {L"!", FACT}, {L"^", POWER}, {L".", PERIOD}, {L",", COMMA}, {L":", COLON}, {L";", SEMICOLON},
    {L"=", EQ}, {L"~", TILDE}, {L"π", SPI}, {L"Ø", SPHI}, {L"|", VERT_LINE}, {L"‹", OLQUOTE}, {L"›", CLQUOTE}, {L"♪", NOTE}, {L"⬳", SAW}, {L"∿", FSIN}, {L"τ", IDENT_t},
    {L"☯", YINYANG}, {L"§", SEQUENCE}, {L"✬", FREQ_MESH}, {L"➡", RET}, {L"♭", FLAT}, {L"♯", SHARP}},
              words = {{L"sin", FSIN}, {L"cos", FCOS}, {L"tan", FTAN}, {L"exp", FEXP}, {L"log", FLOG}, {L"log10", FLOG10}, {L"int", FINT}, {L"sqrt", FSQRT}, {L"asin", FASIN}, {L"acos", FACOS}, {L"atan", FATAN}, {L"abs", FABS}, {L"pi", SPI}, {L"phi", SPHI}, {L"wave", SWAVE}, {L"wave1", SWAVE1}, {L"wave2", SWAVE2}, {L"tone", TONE}, {L"note", NOTE}, {L"sec", SEC}, {L"osc", OSC}, {L"saw", SAW}, {L"saw1", SAW1}, {L"lap", LAP}, {L"hz2oct", HZ2OCT}, {L"magneticring", MAGNETICRING}, {L"rate", RATE},
  ...
  public:
    symbolmap initial = {{L"-", MINUS}, {L">", GT}, {L"<", LT}},
              notes = {{L"do", N_DO}, {L"re", N_RE}, {L"mi", N_MI}, {L"fa", N_FA}, {L"sol", N_SOL}, {L"la", N_LA}, {L"si", N_SI}};

    Symbols() = default;

    symbol is_reserved(wchar_t ch)
    { // check is ch in map -> sym
      return reserved.contains(ch);
    }
    symbol is_reserved(wchar_t ch0, wchar_t ch1)
    { // check is ch in map -> sym
      return reserved.contains(ch0, ch1);
    }

    symbol is_reserved(wstring id)
    { // is a reserved word ?
    ...
    symbolmap getChars() { return chars; }
  };
  ...
  public:
    ...
    map<wstring, symbol> getChars() { return syms.getChars(); }
    ...
    { // -> sym
      ...
      { // ident / res. word / func
        ...
        if ((sym = syms.is_reserved(id)) == SNULL)
        {
          ...
          { // note#b@oct: do, do@3, do#, do#@2
            map<symbol, int> note_val = {{N_DO, 0}, {N_RE, 2}, {N_MI, 4}, {N_FA, 5}, {N_SOL, 7}, {N_LA, 9}, {N_SI, 11}};
      ...
      else
      {
        ...
        { // number: dddd.ddde-dd
          ...
          try
          {
            nval = std::stod(id); // beware of locales
            // printf("num: %ls -> %f\n", id.c_str(), nval);
          }
          catch (...)
          {
  ...
  class Compiler
  {
    ...
    jmp_buf jmp_env;
public:
    map<wstring, symbol> getChars() { return parser->getChars(); }

    void parse_id_eq_expr()
    { // id=expr, id=expr;
      do
      {
        if (getsym() == IDENT)
  ...
  public:
    ...
    string error_msg()
    {
      return err ? string("syntax error near line ") +
                       std::to_string(parser->get_lineno() + 1) +
                       std::to_string(parser->get_current_ch())
                 : string("ok");
    }
  ...
  private:
    ...
    void expr_1(void)
    {
      if (!err)
      {
        ...
        {
          symbol sym_op = sym;
          if (starts_implicit_mult())
          { // not operator-> implicit *, i,e.
            // 2{440}
            expr_2();
            generate(MULT);
          }
...
public:
  ...
  map<wstring, symbol>getSymbols() { return compiler->getChars(); }
  ...
  int sample_rate = 44100, bits_sample = -32;
...
public:
  ...
  string error_msg() { return compiler ? compiler->error_msg() : ""; }

  string get_wave_mesh()
  {
    string s;
  ...
  void generateBuffer(int nSamples, float *buffer)
  {
    double tinc = 2 * M_PI / get_sample_rate(), t = sampleCount * tinc;

    for (auto i = 0; i < nSamples; i++, t += tinc)
      for (auto chan = 0; chan < get_n_channels(); chan++)
        *buffer++ = get_volume() * execute(t, chan);

    sampleCount += nSamples;
    if ((double)sampleCount > get_seconds() * sample_rate)
      sampleCount = 0;
  }

  vector<float>genSamples() {
    vector<float>vb;
    float v, max=-1e30;
    double tinc = 2 * M_PI / get_sample_rate(), t = sampleCount * tinc;

    for (auto i = 0; i < (int)(get_seconds() * get_sample_rate()); i++, t += tinc)
      for (auto chan = 0; chan < get_n_channels(); chan++) {
        v=execute(t, chan);
        if (fabs(v)>max) max=fabs(v);
        vb.push_back(v);
      }
    if (max!=0) max=1; // normalize
    for (auto &v:vb) v*=get_volume() / max;
