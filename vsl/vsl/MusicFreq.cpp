...
//---------------------------------------------------------------------------
Color MusicFreq::Freq2Color(double freq)
{
  ...
  ColorReff c = ColorScaleHSL(MakeRGB(0xff, 0, 0), MakeRGB(0xff, 0, 0xff), ratio);
  return c;
}
...
string MusicFreq::NoteString(int i)
{
  if (i >= 0 && i <= 12)
    return note_str()[i];
  else
...
string MusicFreq::NoteString(double freq)
{
...
string MusicFreq::Freq2StrNote(double freq)
{
...
//---------------------------------------------------------------------------
string MusicFreq::Freq2NoteOct(double freq)
{
  int n, o;
  string no;
  Freq2NoteOct(freq, n, o, no);
  return NoteString(n) + "/" + to_string(o);
}
...
int MusicFreq::Freq2NoteOct(double freq, int &note, int &oct, string &NoteOct,
                            double &err)
{
  if (freq <= 0)
  {
  ...
  // string
  if (NoteOct != "")
    NoteOct = string(note_str()[note]) +
              ((oct < 0) ? to_string(oct) : "+" + to_string(oct));
  // error
...
// string MusicFreq::NoteStr2ColorName(string note)
// {
//   int nnote = std::find(note_str().begin(), note_str().end(), note);
//   double freq = NoteOct2Freq(nnote, 0);
//   Color col = Freq2Color(freq);
//   return col.name();
// }
// string MusicFreq::Freq2ColorName(double freq)
// {
//   Color col = Freq2Color(freq);
//   return col.name();
// }
