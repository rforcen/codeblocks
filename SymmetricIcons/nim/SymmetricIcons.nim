...
{.push cdecl, exportc.}
proc newSymmIcon*(w, preset, color_set:cint) : ref SymmetricIcons =
    box(newSymmetricIcon(w, preset, color_set.uint32))

proc generate*(si : ref SymmetricIcons, iters:cint) =
    si[].generate(iters)
    
proc getImageSize*(si : ref SymmetricIcons) : int =
    si[].image.len

proc getImage*(si : ref SymmetricIcons, data:ptr uint32) =
    copyMem(data, si[].image[0].addr, si[].image.len * 4)

proc writeImage*(si : ref SymmetricIcons, data:ptr uint32, name:cstring) =
    var 
        size = si.len * si.len
        image = newImage(si.len , si.len)

    image.data = newSeq[ColorRGBX](size)
    copyMem(image.data[0].addr, data, size*4)
