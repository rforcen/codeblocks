...
public:
    ...
    SymmetricIcon(int npr = 0) : npr(npr)
    {
        setPreset(npr);
    }
    ...
    vector<RGB>getRGB(vector<uint32_t>&img)
    {
        // convert screen to rgb

        vector<RGB>rgb;
        RGB _rgb;
        rgb.reserve(img.size());
    ...
    double getParam(int preset, int i)
    {
        return this->preset[preset * 7 + i];
    }

    bool generateSymmIcon(int modDisp=1)   // geenrate icon, runs in a thread in 'start'
    {
        iter++;
