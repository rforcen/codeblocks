...
public:
  Mandelbrot(u32 w, u32 h, u32 iters, Complex32 center, Complex32 range)
      : w(w), h(h), iters(iters), center(center), range(range), image(new u32[w * h]),
        cr(Complex32(range.real(), range.real())), rir((range.imag() - range.real())), scale(0.8 * float(w) / h)
  {
  }
...
void test_mandel()
{
  ...
  auto mandel = Mandelbrot(n, n, iters, Complex32(0.5f, 0.0f), Complex32(-2.0f, 2.0f));
